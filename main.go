...
func main() {
	...
	localFilePath := "hd-zero.html" // Path where HTML file will be stored
	...
	finalZIPList := extractZIPUrls(strings.Join(getData, "")) // Extract all ZIP links from HTML content
	...
	// Remove duplicates from a given slice.
	finalZIPList = removeDuplicatesFromSlice(finalZIPList)
	// Loop through all extracted ZIP URLs
	for _, urls := range finalZIPList {
		if isUrlValid(urls) { // Check if the final URL is valid
			downloadZIP(urls, outputDirZIP) // Download the ZIP
		}
	}
}
...
// Downloads a ZIP file from given URL and saves it in the specified directory
func downloadZIP(finalURL, outputDir string) bool {
	...
	if !strings.Contains(contentType, "binary/octet-stream") && !strings.Contains(contentType, "application/zip") && !strings.Contains(contentType, "application/x-zip-compressed") { // Check if it's a ZIP
	...
	if _, err := buf.WriteTo(out); err != nil { // Write buffer contents to file
		log.Printf("Failed to write ZIP to file for %s: %v", finalURL, err)
		return false
	}
...
// Extracts all links to ZIP files from given HTML string
func extractZIPUrls(htmlInput string) []string {
	var zipLinks []string // Slice to hold found ZIP links
		...
		if n.Type == html.ElementNode && n.Data == "a" { // If it's an <a> tag
			for _, attr := range n.Attr {
				if attr.Key == "href" { // Look for href attribute
					...
					if strings.Contains(strings.ToLower(href), ".zip") { // If link points to a ZIP
						zipLinks = append(zipLinks, href) // Add to list
					}
				}
			}
		}
	...
	return zipLinks // Return found ZIP links
}
